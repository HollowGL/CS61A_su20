...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            other = place.ant
            if isinstance(self, ContainerAnt):  # self contains other
                if not self.can_contain(other):
                    assert False, 'Two ants in {0}'.format(place)
                self.contain_ant(other)
                place.ant = self
                Insect.add_to(other, place)
            elif isinstance(other, ContainerAnt):  # other contains self
                if not other.can_contain(self):
                    assert False, 'Two ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        curPlace: Place = self.place
        distance = 0
        while curPlace != beehive:
            if curPlace.bees and distance >= self.min_range and distance <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        targetBuff = self.place.bees[:]
        super().reduce_armor(amount)  # or Ant.reduce_armor(self, amount)
        attack = amount
        if self.armor <= 0:
            attack += self.damage
        for bee in targetBuff:
...
class BodyguardAnt(ContainerAnt):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(armor=kwargs.get('armor', 2))
    # END Problem 9
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if QueenAnt.has_queen and not self.is_queen:
            self.reduce_armor(self.armor)
            return

        super().action(gamestate)
        tmp = self.place
        while tmp.exit is not None:
            tmp = tmp.exit
            if tmp.ant is None:
                continue
            if tmp.ant not in self.inspired_ants:
                self.inspired_ants.append(tmp.ant)
                tmp.ant.damage *= 2
            if not isinstance(tmp.ant, ContainerAnt) or not tmp.ant.contained_ant:
                continue
            if tmp.ant.contained_ant not in self.inspired_ants:
                self.inspired_ants.append(tmp.ant.contained_ant)
                tmp.ant.contained_ant.damage *= 2
...
def make_slow(action, bee):
    ...
    def slow(gamestate):
        if gamestate.time % 2 == 0:
            # bee.action(gamestate)  # infinite recursion
            action(gamestate)
    return slow
    # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while cur_place.entrance is not beehive:
            ...
            for bee in cur_place.bees:
                insect_dict[bee] = cur_dist

            tmp_ant = cur_place.ant
            if tmp_ant is None:
                continue
            insect_dict[tmp_ant] = cur_dist
            if isinstance(tmp_ant, ContainerAnt) and tmp_ant.contained_ant is not None:
                insect_dict[tmp_ant.contained_ant] = cur_dist
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        weak = 0.2 * distance
        down = 0.05 * self.insects_shot
        new_damage = LaserAnt.damage - weak - down
        return new_damage if new_damage > 0 else 0
        # END Problem OPTIONAL
