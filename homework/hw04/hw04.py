...
def has_path(t, phrase):
    ...
    if len(phrase) == 1:
        ...
        res = [has_path(b, phrase[1:]) for b in branches(t)]
        if sum(res):
            return True
    else:
        return False
...
def quadratic(x, a, b, c):
    ...
    def f(t):
        return a*t*t + b*t + c
    middle = -b * 0.5 / a
    if middle <= (lower_bound(x) + upper_bound(x)) / 2:
        m = f(middle) if middle >= lower_bound(x) else f(lower_bound(x))
        n = f(upper_bound(x))
    else:
        m = f(middle) if middle <= upper_bound(x) else f(upper_bound(x))
        n = f(lower_bound(x))
    if a >= 0:
        return interval(m, n)
    else:
...
# r1 = (x1, x2), r2 = (y1, y2)
# par1: x1*y1/(x2+y2), x2*y2/(x1+y1)
# par2: x1*y1/(x1+y1), x2*y2/(x2+y2)
