...
def composer(func=lambda x: x):
    ...
    def func_adder(g):
        "*** YOUR CODE HERE ***"       
        return lambda x: func(g(x)), lambda f: composer(lambda x: func(g(f(x))))  
...
def count_change(total):
    ...
    def cout_hp(n, m):
        if n == 0:
            ...
            return cout_hp(n - 2**m, m) + cout_hp(n, m-1)
    return cout_hp(total, max_two(total))
    ...
    def helper(n, m):
        if n == 0:
            ...
            return helper(n, m + 1) + helper(n - (1 << m), m) # << 位运算符
