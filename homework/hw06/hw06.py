...
class VendingMachine:
    ...
    """
    "*** YOUR CODE HERE ***"
    def __init__(self, drink, price) -> None:
        self.drink = drink
        self.price = price
        self.funds = 0
        self.stock = 0
    ...
    def restock(self, cnt):
        self.stock += cnt
        return f'Current {self.drink} stock: {self.stock}'
    ...
    def vend(self):
        ...
        if self.funds < self.price:
            ...
            self.funds = 0
            self.stock -= 1
            return f'Here is your {self.drink}.'
        else:
            change = self.funds - self.price
            self.funds = 0
            self.stock -= 1
            return f'Here is your {self.drink} and ${change} change.'
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        delta = Mint.current_year - self.year
        if delta <= 50:
            return self.cents
        return self.cents + delta - 50
...
def is_bst(t):
    ...
    if not (b1.label <= t.label and b2.label >= t.label):
        return False
    if not (bst_min(b1) <= t.label and bst_max(b2) >= t.label):
        return False
    return is_bst(b1) and is_bst(b2)
...
def deep_map(f, link):
    ...
    if isinstance(link.first, Link):
        head = deep_map(f, link.first)
    else:
        head = f(link.first)
    rest = deep_map(f, link.rest)
    return Link(head, rest)
